...
fn shade(@builtin(workgroup_id) workgroup_id: vec3<u32>, @builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    var light_cache_cell = query_light_cache(world_position, view.world_position);
    let direct_lighting = evaluate_lighting_from_cache(&rng, &light_cache_cell, world_position, world_normal, wo, material, view.exposure);
    write_light_cache(light_cache_cell, world_position, view.world_position);
